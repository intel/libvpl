# VPL Surface Sharing - Overview of new APIs

VPL API 2.10 introduces new interfaces for sharing surfaces between VPL runtime and other applications, frameworks, and graphics APIs. This functionality is only supported when using the internal memory management model.
...
## Importing and exporting surfaces between VPL runtime and external frameworks
...
A new interface structure ```mfxMemoryInterface``` is defined, which contains function pointers filled in by the VPL runtime. New functions are then called by the application (i.e. a callback interface). 
...
- Defines a new enum ```mfxSurfaceComponent``` to distinguish which internal surface pool frames are imported/exported to/from. This is required in order for VPL runtime to allocate and assign surfaces from the proper internal frame pool.
...
```
 ...
 - Defines a new list of enums prefixed with ```MFX_SURFACE_FLAG_``` which will be used as both inputs and outputs to VPL import/export operations. To avoid need for explicit cast when OR'ing flags together, it is not a typedef.
```
...
- As outputs, these flages indicate what operation VPL runtime performed (shared mapping or copy).
...
As with other VPL implementation capabilities reports, an application may use this information in two ways:
...
// export mfxFrameSurface1 from VPL to a D3D11 texture, zero-copy enabled
...
### Importing a D3D11 texture as a VPL frame for encoding
```
...
// import D3D11 texture into VPL, zero-copy preferred, copy okay
...
### Exporting a frame decoded in VPL to a VAAPI surface
```
// decode frame in VPL
sts = MFXVideoDECODE_DecodeFrameAsync(session, (isDrainingDec ? NULL : &fileInfo->bitstream), NULL, &pmfxDecOutSurface, &syncpD);
...
// export mfxFrameSurface1 from VPL to a VAAPI surface, zero-copy enabled
...
### Importing a VAAPI surface as a VPL frame for encoding
...
// import VAAPI surface into mfxFrameSurface1 for VPL
// set header.surfaceType, device, and texture2D, all other fields should be empty
...
// import VAAPI surface into VPL, zero-copy preferred, copy okay
mfxFrameSurface1 *pmfxEncSurface = nullptr;
...
### Exporting a frame decoded in VPL to an OpenCL image for processing
...
// export mfxFrameSurface1 from VPL to an OpenCL image, zero-copy enabled
...
### Importing an OpenCL image as a VPL frame for encoding
...
// once OpenCL processing is complete, import the surface into VPL
...
// import the OpenCL surface into a VPL surface
mfxFrameSurface1 *pmfxEncSurface = nullptr;
...
For Export case (for both Copy and Shared type) original `mfxFrameSurface1` and Exported `mfxSurfaceXXX` can be managed (i.e. `Release()`) independently, VPL RT manages dependency and keeps all required resources alive.