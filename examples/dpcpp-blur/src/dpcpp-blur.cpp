...
public:
    ...
    int operator()(const cl::sycl::device &device) const override {
        ...
        if (device.is_gpu())
            return 500; // Higher merit for GPU
        if (device.is_cpu())
            return 100; // Select CPU if no GPU available
...
int main(int argc, char *argv[]) {
    ...
    blurred_surface            = {};
    blurred_surface.Info       = mfxVPPParams.vpp.Out;
...
#ifdef BUILD_DPCPP
                sts = vppOutSurface->FrameInterface->Map(vppOutSurface, MFX_MAP_READ);
                if (sts != MFX_ERR_NONE) {
