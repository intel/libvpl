...
mfxF64 cHRD::GetBufferDiviation(mfxU32 targetBitrate) {
    ...
    mfxI64 minTargetFullness =
        std::min<mfxU32>(m_buffSizeInBits / 2, targetBitrate * 2); // half bufsize or 2 sec
...
mfxU16 cHRD::UpdateAndCheckHRD(mfxI32 frameBits, mfxI32 recode, mfxI32 minQuant, mfxI32 maxQuant) {
    ...
    m_minFrameSize =
        (!m_bCBR) ? 0 : (mfxI32)(m_bufFullness + 1 + 1 + m_inputBitsPerFrame - m_buffSizeInBits);
    if (m_minFrameSize < 0)
        m_minFrameSize = 0;
...
mfxStatus ExtBRC::Update(mfxBRCFrameParam* frame_par,
    ...
    if (m_avg.get()) {
        frameSizeLim = std::min(frameSizeLim,
    ...
    if (((m_par.HRDConformance != MFX_BRC_NO_HRD && brcSts != MFX_BRC_OK) ||
        ...
        if (!m_ctx.bPanic && (!oldScene) && bNeedUpdateQP) {
            ...
            if (m_par.HRDConformance != MFX_BRC_NO_HRD && totDiv > 0) {
                ...
                bAbPreriod =
                    (mfxF64)(m_par.bPyr ? 4 : 3) * (mfxF64)m_hrd.GetMaxFrameSize() /
                    m_par.inputBitsPerFrame *
            ...
            quant_new = GetNewQPTotal(totDiv / bAbPreriod / (mfxF64)m_par.inputBitsPerFrame,
                                      dequant_new,
                                      m_ctx.QuantMin,
                                      m_ctx.QuantMax,
                                      m_ctx.Quant,
