...
#define SERIALIZE_INFO_ELEMENT(arrName, name)                                                      \
    {                                                                                              \
        sstr << prefix << MSDK_STRING(#arrName) << MSDK_STRING("[") << arrIdx << MSDK_STRING("].") \
             << MSDK_STRING(#name) << MSDK_STRING(":") << info.arrName[arrIdx].name << std::endl;  \
    }
#define SERIALIZE_INFO_ARRAY_ELEMENT(arrName, name)                                                \
    {                                                                                              \
        sstr << prefix << MSDK_STRING(#arrName) << MSDK_STRING("[") << arrIdx << MSDK_STRING("].") \
             << MSDK_STRING(#name) << MSDK_STRING(":");                                            \
...
#define SERIALIZE_INFO(name) \
    { sstr << prefix << MSDK_STRING(#name) << MSDK_STRING(":") << info.name << std::endl; }
    ...
    switch (pExtBuffer->BufferId) {
        ...
        case MFX_EXTBUFF_VPP_COMPOSITE: {
            ...
            for (int i = 0; i < info.NumInputStream; i++) {
                ...
                sstr << prefix << MSDK_STRING("InputStream[") << i << MSDK_STRING("].");
                SerializeVPPCompInputStream(sstr, sstr.str(), info.InputStream[i]);
            }
        } break;
...
void CParametersDumper::SerializeVPPCompInputStream(msdk_ostream& sstr,
                                                    msdk_string prefix,
...
mfxStatus CParametersDumper::DumpLibraryConfiguration(msdk_string fileName,
    ...
    try {
        ...
        sstr << MSDK_STRING("Configuration settings (fields from API ") << MFX_VERSION_MAJOR
             << MSDK_STRING(".") << MFX_VERSION_MINOR << MSDK_STRING(")\n");
