...
void PrintInfo(sInputParams* pParams, mfxVideoParam* pMfxParams, MFXVideoSession* pMfxSession) {
    ...
    const msdk_char* sImpl =
        (isHWlib) ? MSDK_STRING("hw")
                  : (pParams->bUseVPLLib == true) ? MSDK_STRING("VPL SW") : MSDK_STRING("sw");
#else
...
mfxStatus ParseGUID(msdk_char strPlgGuid[MSDK_MAX_FILENAME_LEN], mfxU8 DataGUID[16]) {
    ...
    for (i = 0; i != 16; i++) {
        hex = 0;
#if defined(_WIN32) || defined(_WIN64)
        if (1 != _stscanf_s(uid + 2 * i, "%2x", &hex))
#else
...
#if defined(_WIN32) || defined(_WIN64)
        if (hex == 0 && (uid + 2 * i != _tcsstr(uid + 2 * i, "00")))
#else
...
#ifdef ENABLE_MCTF
    if (isInput) {
        pAllocator->pExtBuffersStorageSurfaceIn[streamIndex].resize(
            nFrames * MAX_NUM_OF_ATTACHED_BUFFERS_FOR_IN_SUFACE);
    }
#endif
...
#ifdef ENABLE_MCTF
        if (isInput) {
            pSurfaces[i].Data.ExtParam = &(pAllocator->pExtBuffersStorageSurfaceIn[streamIndex].at(
                i * MAX_NUM_OF_ATTACHED_BUFFERS_FOR_IN_SUFACE));
