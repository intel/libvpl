...
mfxStatus ConfigVideoEnhancementFilters(sInputParams* pParams,
    ...
    if (VPP_FILTER_ENABLED_CONFIGURED == pParams->mctfParam[paramID].mode) {
        ...
        pResources->mctfConfig.BitsPerPixelx100k =
    ...
    if (VPP_FILTER_ENABLED_CONFIGURED == pParams->frcParam[paramID].mode) {
        ...
        pResources->frcConfig.Algorithm =
    ...
    if (VPP_FILTER_ENABLED_CONFIGURED == pParams->videoSignalInfoParam[paramID].mode) {
        ...
        pVppParam->ExtParam[pVppParam->NumExtParam++] =
            (mfxExtBuffer*)&(pResources->videoSignalInfoConfig);
    }
    ...
    if (VPP_FILTER_ENABLED_CONFIGURED == pParams->mirroringParam[paramID].mode) {
        ...
        pVppParam->ExtParam[pVppParam->NumExtParam++] =
            (mfxExtBuffer*)&(pResources->mirroringConfig);
    }
    ...
    if (VPP_FILTER_ENABLED_CONFIGURED == pParams->deinterlaceParam[paramID].mode) {
        ...
        pResources->deinterlaceConfig.TelecinePattern =
            pParams->deinterlaceParam[paramID].tc_pattern;
        pResources->deinterlaceConfig.TelecineLocation = pParams->deinterlaceParam[paramID].tc_pos;

        pVppParam->ExtParam[pVppParam->NumExtParam++] =
            (mfxExtBuffer*)&(pResources->deinterlaceConfig);
    }
    if (0 != pParams->rotate[paramID]) {
        ...
        pVppParam->ExtParam[pVppParam->NumExtParam++] =
            (mfxExtBuffer*)&(pResources->rotationConfig);
    }
    if (pParams->bScaling) {
    ...
    if (pParams->bChromaSiting) {
        ...
        pVppParam->ExtParam[pVppParam->NumExtParam++] =
            (mfxExtBuffer*)&(pResources->chromaSitingConfig);
    }
#endif
    ...
    if (VPP_FILTER_ENABLED_CONFIGURED == pParams->multiViewParam[paramID].mode) {
        ...
        pResources->multiViewConfig.View =
        ...
        pVppParam->ExtParam[pVppParam->NumExtParam++] =
            (mfxExtBuffer*)&(pResources->multiViewConfig);
    }
    ...
    if (VPP_FILTER_ENABLED_CONFIGURED == pParams->compositionParam.mode) {
        ...
        pResources->compositeConfig.InputStream =
            new mfxVPPCompInputStream[pResources->compositeConfig.NumInputStream];
        memset(pResources->compositeConfig.InputStream,
               0,
               sizeof(mfxVPPCompInputStream) * pResources->compositeConfig.NumInputStream);

        for (int i = 0; i < pResources->compositeConfig.NumInputStream; i++) {
            pResources->compositeConfig.InputStream[i].DstX =
                pParams->compositionParam.streamInfo[i].compStream.DstX;
            pResources->compositeConfig.InputStream[i].DstY =
                pParams->compositionParam.streamInfo[i].compStream.DstY;
            pResources->compositeConfig.InputStream[i].DstW =
                pParams->compositionParam.streamInfo[i].compStream.DstW;
            pResources->compositeConfig.InputStream[i].DstH =
            ...
            if (pParams->compositionParam.streamInfo[i].compStream.GlobalAlphaEnable != 0) {
                pResources->compositeConfig.InputStream[i].GlobalAlphaEnable =
                    pParams->compositionParam.streamInfo[i].compStream.GlobalAlphaEnable;
                pResources->compositeConfig.InputStream[i].GlobalAlpha =
            ...
            if (pParams->compositionParam.streamInfo[i].compStream.LumaKeyEnable != 0) {
                pResources->compositeConfig.InputStream[i].LumaKeyEnable =
                    pParams->compositionParam.streamInfo[i].compStream.LumaKeyEnable;
                pResources->compositeConfig.InputStream[i].LumaKeyMin =
                    pParams->compositionParam.streamInfo[i].compStream.LumaKeyMin;
                pResources->compositeConfig.InputStream[i].LumaKeyMax =
            ...
            if (pParams->compositionParam.streamInfo[i].compStream.PixelAlphaEnable != 0) {
                pResources->compositeConfig.InputStream[i].PixelAlphaEnable =
                    pParams->compositionParam.streamInfo[i].compStream.PixelAlphaEnable;
            }
        } // for (int i = 0; i < pResources->compositeConfig.NumInputStream; i++)

        pVppParam->ExtParam[pVppParam->NumExtParam++] =
            (mfxExtBuffer*)&(pResources->compositeConfig);
    }
