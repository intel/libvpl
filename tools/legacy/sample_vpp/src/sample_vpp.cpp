...
#endif
{
    ...
    if (Params.compositionParam.mode == VPP_FILTER_ENABLED_CONFIGURED) {
        ...
        // And if not in VPL
        if ((Params.fccSource == MFX_FOURCC_I420 && (Params.bUseVPLLib == false)) ||
    ...
    do {
        ...
        while (MFX_ERR_NONE <= sts || MFX_ERR_MORE_DATA == sts || bDoNotUpdateIn) {
            ...
            if (bAttachMctfBuffer) {
                ...
                MctfRTParams =
                    GetMctfParamBuffer<mfxExtVppMctf, MFX_EXTBUFF_VPP_MCTF>(pInSurf[nInStreamInd]);
                WipeOutExtParams(pInSurf[nInStreamInd],
                                 true,
                                 MAX_NUM_OF_ATTACHED_BUFFERS_FOR_IN_SUFACE);
            }
            else {
            ...
            if (Params.use_extapi) {
                ...
                if (bAttachMctfBuffer && MctfRTParams) {
                    ...
                    MctfRTParams->BitsPerPixelx100k =
                        mfxU32(MCTF_AUTO_BPP * MCTF_BITRATE_MULTIPLIER);
                    MctfRTParams->Deblocking   = MFX_CODINGOPTION_OFF;
                    MctfRTParams->TemporalMode = MCTF_TEMPORAL_2REF_MODE;
    #endif

                    if (pInSurf[nInStreamInd]->Data.NumExtParam >=
                        MAX_NUM_OF_ATTACHED_BUFFERS_FOR_IN_SUFACE) {
                        msdk_printf(MSDK_STRING(
                            "the extended buffer is not created; nothing can be attached; exit.\n"));
                        sts = MFX_ERR_UNDEFINED_BEHAVIOR;
                        MSDK_BREAK_ON_ERROR(sts);
                    }
                    else
                        pInSurf[nInStreamInd]
                            ->Data.ExtParam[pInSurf[nInStreamInd]->Data.NumExtParam++] =
                            reinterpret_cast<mfxExtBuffer*>(MctfRTParams);
                }
                else {
                    if (!MctfRTParams && bAttachMctfBuffer) {
                        msdk_printf(MSDK_STRING(
                            "the extended buffer is not created; nothing will be attached\n"));
                        sts = MFX_ERR_UNDEFINED_BEHAVIOR;
                        MSDK_BREAK_ON_ERROR(sts);
                    }
                }
                ...
                if (Params.rtHue.isEnabled || Params.rtSaturation.isEnabled ||
                    ...
                    pOutSurf->Data.NumExtParam =
                        (mfxU16)m_extBuffPtrStorageForOutputSurf[pOutSurf].size();
                }
                nOutFrames++;
#endif
