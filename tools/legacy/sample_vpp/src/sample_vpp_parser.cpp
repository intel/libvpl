...
void vppPrintHelp(const msdk_char *strAppName, const msdk_char *strErrorMessage) {
...
#if (MFX_VERSION >= 2000)
    msdk_printf(
        MSDK_STRING("   [-lib  type]        - type of used library. sw, hw, vpl (def: sw)\n"));
    ...
    msdk_printf(MSDK_STRING(
        "   [-scc  format]              - format (FourCC) of src video (def: nv12. support i420|nv12|yv12|yuy2|rgb565|rgb3|rgb4|imc3|yuv400|yuv411|yuv422h|yuv422v|yuv444|uyvy|ayuv|p010|y210|y410)\n"));
#if (MFX_VERSION >= 2000)
    msdk_printf(MSDK_STRING("                                 vpl supports i420|i010|bgra\n"));
#endif
    msdk_printf(MSDK_STRING(
...
#if (MFX_VERSION >= 1027)
    msdk_printf(MSDK_STRING(
        "   [-dcc format]               - format (FourCC) of dst video (def: nv12. support i420|nv12|yuy2|rgb4|rgbp|yv12|ayuv|a2rgb10|y210|y410)\n"));
    #if (MFX_VERSION >= 2000)
    msdk_printf(MSDK_STRING("                                 vpl supports i420|i010|bgra\n"));
    #endif
#else
    msdk_printf(MSDK_STRING(
        "   [-dcc format]               - format (FourCC) of dst video (def: nv12. support i420|nv12|yuy2|rgb4|rgbp|yv12|ayuv|a2rgb10)\n"));
#endif
    msdk_printf(MSDK_STRING(
...
#if (MFX_VERSION >= 2000)
    msdk_printf(
        MSDK_STRING("                                             vpl supports i420|i010|bgra\n"));
#endif
    msdk_printf(MSDK_STRING(
    ...
    msdk_printf(
        MSDK_STRING("   [-denoise (level)]  - enable denoise algorithm. Level is optional \n"));
...
#if (MFX_VERSION >= 2000)
    else if (0 == msdk_stricmp(strInput, MSDK_STRING("bgra"))) {
        fourcc = MFX_FOURCC_RGB4;
    }
#endif
    ...
    for (mfxU8 i = 1; i < nArgNum; i++) {
        MSDK_CHECK_POINTER(strInput[i], MFX_ERR_NULL_PTR);
        {
            ...
            // aya: altenative and simple way to enable deinterlace
                ...
                for (int ii = 0; ii < 2; ii++) {
                    ...
                    if (msdk_strcmp(strInput[i + ii], MSDK_STRING("vtop")) == 0) {
                        ...
                        msdk_strcmp(MSDK_STRING("Unknown Chroma siting flag %s"), strInput[i + ii]);
                }
            ...
            //-----------------------------------------------------------------------------------
...
#if (MFX_VERSION >= 2000)
                // Check bUseVPLLib here because "-lib vpl" might be located after "-dcc"
                // Need this to prevent from output fourcc is changed by force in coming lines
                if (pParams->bUseVPLLib == false) {
                    for (int j = i + 1; j < nArgNum; j++) {
                        if (0 == msdk_strcmp(strInput[j], MSDK_STRING("-lib"))) {
                            VAL_CHECK(1 + j == nArgNum);
                            j++;
                            if (0 == msdk_strcmp(strInput[j], MSDK_STRING("vpl"))) {
...
#if (MFX_VERSION >= 2000)
                if ((pParams->bUseVPLLib == false &&
                     MFX_FOURCC_I420 == pParams->frameInfoOut[0].FourCC) ||
                ...
                VAL_CHECK(1 + i == nArgNum);
                i++;
                msdk_strncopy_s(pParams->strPlgGuid,
                                MSDK_MAX_FILENAME_LEN,
                                strInput[i],
                                MSDK_MAX_FILENAME_LEN - 1);
